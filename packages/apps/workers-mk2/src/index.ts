...
export default {
  async fetch(
    ...
    try {
      const doId =
        env.CHAT_SESSION.idFromName(
          chatId.toString()
        );

      const stub =
        env.CHAT_SESSION.get(doId);
        ...
        stub
          ...
          .then(async (res) => {
            if (!res.ok) {
              ...
              console.error(
                `DO returned error status ${res.status}:`,
                errText
              );
            }
          })
          .catch((err) => {
            console.error(
              `Failed to reach DO for chat ${chatId}:`,
              err
            );
          })
      );
...
export class ChatbotSessionDO {
    ...
    try {
      ...
      this.state.waitUntil(
        this.queue =
          this.queue
            .then(() =>
              this.processTelegramUpdate(
                update
              )
            )
            .catch((err) => {
              console.error(
                "Unhandled error in DO task execution:",
                err
              );
            })
      );
  ...
  private async processTelegramUpdate(
    ...
    console.log(
      "1. Received Telegram update payload:",
    ...
    try {
      ...
      console.log(
        "3. Sending initial 'thinking' message to Telegram..."
      );
              ...
              await this.sendTelegram(
                "editMessageText",
                {
                  ...
                  message_id:
                    tempMsgId,
                  text:
                    "📖 נא לציין את השאלה או המקור לחיפוש בסיפריא."
                }
              );
            }
              ...
              await this.sendTelegram(
                "editMessageText",
                {
                  ...
                  message_id:
                    tempMsgId,
                  text:
                    "⚠️ לא הוגדר חיבור עבור NEWS_SERVICE."
                }
              );
            }
            ...
            await this.sendTelegram(
              "editMessageText",
              {
                ...
                message_id:
                  tempMsgId,
                text:
                  "📰 ששון אוסף ומסנן את מבזקי החדשות האחרונים עבור כבוד הרב..."
              }
            );
          }

          this.state.waitUntil(
            this.env.NEWS_SERVICE
              .fetch(
                "http://news.local/",
                {
                  method: "POST",
              ...
              await this.sendTelegram(
                "editMessageText",
                {
                  ...
                  message_id:
                    tempMsgId,
                  text:
                    "⚠️ לא הוגדר חיבור עבור ZMAN_SERVICE."
                }
              );
            }
            ...
            await this.sendTelegram(
              "editMessageText",
              {
                ...
                message_id:
                  tempMsgId,
                text:
                  "📊 ששון אוסף את נתוני מזג האוויר וזמני ההלכה עבור כבוד הרב..."
              }
            );
          }

          this.state.waitUntil(
            this.env.ZMAN_SERVICE
              .fetch(
                "http://zman.local/",
                {
                  method: "POST",
              ...
              await this.sendTelegram(
                "editMessageText",
                {
                  ...
                  message_id:
                    tempMsgId,
                  text:
                    "⚠️ לא הוגדר חיבור עבור LNEWS_SERVICE."
                }
              );
            }
            ...
            await this.sendTelegram(
              "editMessageText",
              {
                ...
                message_id:
                  tempMsgId,
                text:
                  "📺 ששון בודק שידורים חיים בערוץ 14 וב-i24NEWS עבור כבוד הרב..."
              }
            );
          }

          this.state.waitUntil(
            this.env.LNEWS_SERVICE
              .fetch(
                "http://lnews.local/",
                {
                  method: "POST",
              ...
              await this.sendTelegram(
                "editMessageText",
                {
                  ...
                  message_id:
                    tempMsgId,
                  text:
                    "⚠️ לא הוגדר חיבור עבור MOVI_SERVICE."
                }
              );
            }
            ...
            await this.sendTelegram(
              "editMessageText",
              {
                ...
                message_id:
                  tempMsgId,
                text:
                  "🎬 ששון מחפש את הסרטון החדש מ-24 השעות האחרונות עבור כבוד הרב..."
              }
            );
          }

          this.state.waitUntil(
            this.env.MOVI_SERVICE
              .fetch(
                "http://movi.local/",
                {
                  method: "POST",
      ...
      // ======================================================================
            ...
            await this.sendTelegram(
              "editMessageText",
              {
                ...
                message_id:
                  tempMsgId,
                text:
                  "🔇 שירות הזיהוי הקולי כבוי כעת. ניתן להפעילו עם /son."
              }
            );
          }
        ...
        const sttService =
          this.env.STT_SERVICE;

        if (!sttService) {
          throw new Error(
            "STT_SERVICE binding missing."
          );
        }
        ...
        const fileInfo =
          (await fileInfoRes.json()) as TelegramGetFileResult;
        ...
        if (
          ...
          throw new Error(
            "Failed to get voice file path from Telegram."
          );
        }
        ...
        const audioBuffer =
          await voiceFileRes.arrayBuffer();

        const ssttRes =
          await sttService.fetch(
            "http://sstt.local/",
            {
              method: "POST",
              headers: {
            ...
            await this.sendTelegram(
              "editMessageText",
              {
                ...
                message_id:
                  tempMsgId,
                text:
                  "⚠️ לא הצלחתי לפענח מילים ברורות בהודעה הקולית."
              }
            );
          }
      ...
      let messages: any[] =
        (await this.state.storage.get<any[]>(
          "history"
        )) || [];
      ...
      const tools = [
        {
          ...
          function: {
            ...
            description:
              "Search the web for up-to-date and real-time information on any topic.",
      ...
      while (
        ...
        console.log(
          `Agent Loop Turn ${round}/${MAX_SEARCH_ROUNDS}...`
        );

        const aiResponse =
          await this.executeLLMPipeline(
            activeMessages,
            tools,
            currentProvider
          );
        ...
        if (
          ...
          aiResponse.tool_calls.length >
            0
        ) {
          const toolCall =
            aiResponse.tool_calls[0];
          ...
          if (
              ...
              await this.sendTelegram(
                "editMessageText",
                {
                  ...
                  message_id:
                    tempMsgId,
                  text:
                    `🌐 מבצע חיפוש מעמיק ברשת (${round}/${MAX_SEARCH_ROUNDS}) עבור כבוד הרב...`
                }
              );
            }
            ...
            try {
              ...
              searchResultsStr =
                "שגיאת חיפוש: החיפוש ברשת נכשל. אנא השב על בסיס הידע הקיים שלך.";
            }

            const toolCallId =
              toolCall.id ||
              `call_${Date.now()}_${round}`;
      ...
      if (!finalAnswer) {
          ...
          await this.sendTelegram(
            "editMessageText",
            {
              ...
              message_id:
                tempMsgId,
              text:
                "✍️ מנסח תשובה מקיפה עבור כבוד הרב..."
            }
          );
        }
        ...
        if (!finalAnswer) {
          activeMessages.push({
            ...
            content:
              "אנא נסח כעת את התשובה המלאה והסופית עבור כבוד הרב מתוך כל תוצאות החיפוש שנאספו לעיל."
          });
          ...
          finalAnswer =
            retryAi.response
              ?.trim() ||
            "לא הצלחתי לעבד את תוצאות החיפוש. אנא נסה שוב.";
        }
      }

      console.log(
        "10. Final Answer calculated:",
        finalAnswer
      );
          ...
          await this.sendTelegram(
            "editMessageText",
            {
              ...
              message_id:
                tempMsgId,
              text:
                "⚠️ אירעה שגיאה במהלך עיבוד השיחה: " +
                errMsg
            }
          );
        } catch (teleErr) {
  ...
  private async streamTTSVoiceToTelegram(
    ...
    if (!ttsResponse.ok) {
      ...
      throw new Error(
        `TTSS returned ${ttsResponse.status}: ${errorText}`
      );
    }

    if (!ttsResponse.body) {
      throw new Error(
        "TTSS returned no readable stream."
      );
    }
    ...
    const prefix =
      ...
      `--${boundary}${CRLF}` +
      `Content-Disposition: form-data; name="voice"; filename="voice.mp3"${CRLF}` +
      ...
      new ReadableStream<Uint8Array>({
        async start(
          ...
          try {
            ...
            console.error(
              "[TTS STREAM] Multipart stream error:",
              error
            );
    ...
    if (
      ...
      throw new Error(
        `Telegram streaming upload failed: ${telegramResponse.status} ${telegramText}`
      );
    }

    let telegramJson: any;
    ...
    try {
      ...
      throw new Error(
        `Telegram returned invalid JSON: ${telegramText}`
      );
    }
  ...
  private async callGeminiAPI(
    ...
    if (!response.ok) {
      ...
      throw new Error(
        "Google Gemini API returned status " +
          response.status +
          ". Details: " +
          errText
      );
  ...
  private async callNvidiaAPI(
    ...
    return {
      ...
      provider:
        "nvidia" as LLMProvider
    };
  }

  private stripMarkdownAndEmojis(
    text: string
  ): string {
    return text
      .replace(
  ...
  private trimHistorySafely(
    messages: any[],
    maxNonSystem: number = 15
  ): any[] {
    if (
      messages.length === 0
    ) {
